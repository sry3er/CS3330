...
wire loadUse:1;
loadUse = (E_icode == MRMOVQ && (E_dstE == d_srcA || E_dstE == d_srcB));

f_icode = i10bytes[4..8];
f_ifun = i10bytes[0..4];
wire need_regs:1, need_immediate:1;

need_regs = f_icode in {RRMOVQ, IRMOVQ, OPQ, CMOVXX, RMMOVQ, MRMOVQ};
need_immediate = f_icode in {IRMOVQ, RMMOVQ, MRMOVQ};
...
f_Stat = [
	...
	f_icode in {NOP, RRMOVQ, IRMOVQ, OPQ, CMOVXX, RMMOVQ, MRMOVQ} : STAT_AOK;
	1 : STAT_INS;
];
stall_P = (f_Stat != STAT_AOK) || loadUse; # so that we see the same final PC as the yis tool
...
d_valA = [
	reg_srcA == REG_NONE : 0;
	M_icode == MRMOVQ && reg_srcA == m_dstE : m_valM;
	M_icode == MRMOVQ && reg_srcA == W_dstE : W_valM;
...
d_valB = [
	reg_srcB == REG_NONE : 0;
	M_icode == MRMOVQ && reg_srcB == m_dstE : m_valM;
	M_icode == MRMOVQ && reg_srcB == W_dstE : W_valM;
